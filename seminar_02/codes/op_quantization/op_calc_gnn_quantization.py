...
class ScatterObservedPorts(layers.Layer):
    """Scatter a subset of observed features into a dense graph tensor.

    Args:
        node_indices (list[int]): Graph node indices that correspond to
            observed ports. These positions receive the observed features.
        num_nodes (int): Total number of graph nodes in the dense graph input.
            If larger values are used, the scatter matrix is wider and consumes
            more memory; if smaller values are used than expected, model
            reconstruction fails due to shape incompatibility.
        **kwargs: Additional Keras layer keyword arguments.

    Returns:
        None: This class is a Keras layer constructor.

    Raises:
        ValueError: If the incoming feature width does not match the number of
            observed indices.
    """
    ...
    def build(self, input_shape):
        """Build scatter matrix used to place observed features on graph nodes.

        Args:
            input_shape (tuple[int, ...]): Input tensor shape provided by
                Keras. The last axis must equal the observed feature count.

        Returns:
            None: Initializes layer state in-place.

        Raises:
            ValueError: If ``input_shape[-1]`` does not match observed width.
        """
        expected = input_shape[-1]
        if expected is not None and expected != self._num_observed:
            raise ValueError(
                f"Expected {self._num_observed} observed features, got {expected}."
            )
        dtype = tf.dtypes.as_dtype(self.dtype or tf.float32)
        scatter = tf.one_hot(self.node_indices, depth=self.num_nodes, dtype=dtype)
        self._scatter_matrix = scatter
        super().build(input_shape)

    def call(self, inputs):
        """Scatter observed inputs onto the full graph node dimension.

        Args:
            inputs (tf.Tensor): Observed features with shape
                ``(batch, num_observed_ports)``.

        Returns:
            tf.Tensor: Dense node-feature tensor with shape
            ``(batch, num_nodes, 1)``.

        Raises:
            tf.errors.InvalidArgumentError: If matrix multiplication fails due
                to incompatible tensor shapes.
        """
    ...
    def compute_output_shape(self, input_shape):
        """Infer the output shape produced by ``call``.

        Args:
            input_shape (tuple[int, ...]): Layer input shape.

        Returns:
            tuple[int | None, int, int]: Output shape
            ``(batch, num_nodes, 1)``.
    ...
    def get_config(self):
        """Serialize layer configuration for model saving/loading.

        Args:
            None: Uses instance attributes only.
...
MODELS = {
    3: {
        "model_path": "/media/matheus/SSD-2/matheus/results/fluidra/nas/v2/gnn/nas_gnn_3_ports_v2.0_3_ports/optuna_study/models/top_1_trial_101.keras",
        "batch_size": 512,
        "scaler_path": "/media/matheus/SSD-2/matheus/results/fluidra/nas/v2/gnn/nas_gnn_3_ports_v2.0_3_ports/optuna_study/scaler/top_1_trial_101.pkl",
    },
    4: {
        "model_path": "/media/matheus/SSD-2/matheus/results/fluidra/nas/v2/gnn/nas_gnn_4_ports_v2.0_4_ports/optuna_study/models/top_1_trial_184.keras",
        "batch_size": 256,
        "scaler_path": "/media/matheus/SSD-2/matheus/results/fluidra/nas/v2/gnn/nas_gnn_4_ports_v2.0_4_ports/optuna_study/scaler/top_1_trial_184.pkl",
    },
    5: {
        "model_path": "/media/matheus/SSD-2/matheus/results/fluidra/nas/v2/gnn/nas_gnn_5_ports_v2.0_5_ports/optuna_study/models/top_1_trial_487.keras",
        "batch_size": 256,
        "scaler_path": "/media/matheus/SSD-2/matheus/results/fluidra/nas/v2/gnn/nas_gnn_5_ports_v2.0_5_ports/optuna_study/scaler/top_1_trial_487.pkl",
    },
    6: {
        "model_path": "/media/matheus/SSD-2/matheus/results/fluidra/nas/v2/gnn/nas_gnn_6_ports_v2.0_6_ports/optuna_study/models/top_1_trial_928.keras",
        "batch_size": 256,
        "scaler_path": "/media/matheus/SSD-2/matheus/results/fluidra/nas/v2/gnn/nas_gnn_6_ports_v2.0_6_ports/optuna_study/scaler/top_1_trial_928.pkl",
    },
    7: {
        "model_path": "/media/matheus/SSD-2/matheus/results/fluidra/nas/v2/gnn/nas_gnn_7_ports_v2.0_7_ports/optuna_study/models/top_1_trial_1013.keras",
        "batch_size": 256,
        "scaler_path": "/media/matheus/SSD-2/matheus/results/fluidra/nas/v2/gnn/nas_gnn_7_ports_v2.0_7_ports/optuna_study/scaler/top_1_trial_1013.pkl",
    },
    10: {
        "model_path": "/media/matheus/SSD-2/matheus/results/fluidra/nas/v2/gnn/nas_gnn_10_ports_v2.0_10_ports/optuna_study/models/top_1_trial_1005.keras",
        "batch_size": 256,
        "scaler_path": "/media/matheus/SSD-2/matheus/results/fluidra/nas/v2/gnn/nas_gnn_10_ports_v2.0_10_ports/optuna_study/scaler/top_1_trial_1005.pkl",
    },
    15: {
        "model_path": "/media/matheus/SSD-2/matheus/results/fluidra/nas/v2/gnn/nas_gnn_15_ports_v2.0_15_ports/optuna_study/models/top_1_trial_1036.keras",
        "batch_size": 256,
        "scaler_path": "/media/matheus/SSD-2/matheus/results/fluidra/nas/v2/gnn/nas_gnn_15_ports_v2.0_15_ports/optuna_study/scaler/top_1_trial_1036.pkl",
    },
}
...
def plot_results(results_by_dataset: dict[str, list[dict[str, float]]]) -> None:
    ...
    for dataset_name, rows in results_by_dataset.items():
        ...
        plt.plot(x_ports, [row["op_q4"] for row in sorted_rows], marker="^", linewidth=1.9, label="GNN (Q=4)")
        plt.plot(x_ports, [row["op_q8"] for row in sorted_rows], marker="v", linewidth=1.9, label="GNN (Q=8)")
        plt.plot(x_ports, [row["op_q16"] for row in sorted_rows], marker="D", linewidth=1.9, label="GNN (Q=16)")
        plt.plot(x_ports, [row["op_q32"] for row in sorted_rows], marker="P", linewidth=1.9, label="GNN (Q=32)")
        ...
        plot_path = os.path.join(PLOTS_DIR, f"op_vs_ports_{dataset_name}_gnn_quantization.png")
        plt.savefig(plot_path, dpi=300)
        plt.close()
        print(f"Saved plot to {plot_path}")


def run_gnn_quantized_op_evaluation() -> None:
    """Run GNN OP evaluation with paper-style embedding quantization sweep.

    The pipeline executes these steps:
    1. Load evaluation datasets and ideal OP baselines.
    2. For each observed-port budget, load the corresponding GNN model.
...
run_gnn_quantized_op_evaluation()
