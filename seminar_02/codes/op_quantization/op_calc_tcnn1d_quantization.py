...
def preprocess_model_inputs(x_values: np.ndarray) -> np.ndarray:
    """Reshape scaled features for TCNN1D model input format.
    ...
    """
    if x_values.ndim != 2:
        raise ValueError(f"Expected rank-2 input before TCNN1D reshape, got shape {x_values.shape}")
...
def plot_results(results_by_dataset: dict[str, list[dict[str, float]]]) -> None:
    ...
    for dataset_name, rows in results_by_dataset.items():
        ...
        plt.plot(
            ...
            marker="s",
            linewidth=2.2,
            label="TCNN1D (No Quant)",
        )
        plt.plot(x_ports, [row["op_q4"] for row in sorted_rows], marker="^", linewidth=1.9, label="TCNN1D (Q=4)")
        plt.plot(x_ports, [row["op_q8"] for row in sorted_rows], marker="v", linewidth=1.9, label="TCNN1D (Q=8)")
        plt.plot(x_ports, [row["op_q16"] for row in sorted_rows], marker="D", linewidth=1.9, label="TCNN1D (Q=16)")
        plt.plot(x_ports, [row["op_q32"] for row in sorted_rows], marker="P", linewidth=1.9, label="TCNN1D (Q=32)")
        ...
        plot_path = os.path.join(PLOTS_DIR, f"op_vs_ports_{dataset_name}_tcnn1d_quantization.png")
        plt.savefig(plot_path, dpi=300)
        plt.close()
        print(f"Saved plot to {plot_path}")


def run_tcnn1d_quantized_op_evaluation() -> None:
    """Run TCNN1D OP evaluation with paper-style embedding quantization sweep.

    The pipeline executes these steps:
    1. Load evaluation datasets and ideal OP baselines.
    2. For each observed-port budget, load the corresponding TCNN1D model.
...
run_tcnn1d_quantized_op_evaluation()
